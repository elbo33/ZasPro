...
As built (ADR 0010, 0011): `zaspro.knowledge` — `aggregate` (materialise `exercise_topics` over the touch set), `agent` (Protocol; `ClaudeKnowledgeAgent` `claude-opus-5`, offline `StubKnowledgeAgent`; prompt `m4-know-v3`), `extract` (business-rule validation, one `EXTRACT_KNOWLEDGE` job per topic), `export`, `run`. Migrations `0008`–`0011`.

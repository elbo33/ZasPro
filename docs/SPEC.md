...
As built (ADR 0010, 0011): `zaspro.knowledge` — `aggregate` (materialise `exercise_topics` over the touch set), `agent` (Protocol; `ClaudeKnowledgeAgent` `claude-opus-5`, streaming, two calls per topic — structure then pedagogy — with a hard `stop_reason == max_tokens` check that fails the job rather than persisting a partial spec; an intermittent `<parameter>` pseudo-syntax malformation in the tool call is re-sampled up to twice, never unpacked; `thinking` is toggleable; offline `StubKnowledgeAgent`; prompt `m4-know-v4`), `extract` (business-rule validation, one `EXTRACT_KNOWLEDGE` job per topic), `export`, `run` (`--all` / `--reset` snapshot-then-wipe / `--no-thinking` / per-topic timing). `zaspro.jobs.PermanentJobError` fails a job without spending its retry budget; transient errors still retry. Migrations `0008`–`0011`.

* **Misconceptions are not suppressed for lack of a citation.** Exam papers do not state student errors, so a "material-supported only" rule returns almost nothing (v2: 7 across 5 topics, four returning zero). The agent emits every real error it can name, labels each with `source_kind` (`MARKING_SCHEME` / `INFORMATOR` / `DISTRACTOR_INFERENCE` / `AGENT_INFERENCE` / `UNSOURCED`), and the low-provenance ones (`AGENT_INFERENCE`, `UNSOURCED`) raise a `knowledge_flags` GAP row routed to review. Human approval in the dashboard is the verification step. Concepts / formulas / methods / examples / objectives keep the strict "only what the material shows" rule.
* **One review card per topic.** A `KNOWLEDGE_SPEC` `ReviewItem` per topic carries the whole spec; `record_decision` APPROVE marks every non-rejected item `APPROVED`, REJECT throws the spec out, EDIT (`reject_items` / `unreject_items`) toggles individual items and leaves the card open. `knowledge_extractions` is one row per topic (last extraction + approval/export state). Dashboard: `/knowledge` index + the `KNOWLEDGE_SPEC` branch of the review card (keys `a`/`E`/`x`/`j`/`k`/`r`/`s`).
* **Git holds the record, the DB is the working store.** An approved topic exports to `knowledge/topics/<official_requirement_code>.yaml` (human-readable, diffable, one file per topic). The committed file is the freeze: `extract_topic` refuses to re-run a topic that has one without `force=True`. The database should be rebuildable from git (`knowledge/`, `sources/`, the migration chain) plus the source documents; `scripts/backup.sh` / `restore.sh` are convenience only and `backups/` is gitignored.

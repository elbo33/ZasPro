...
As built (ADR 0010, 0011): `zaspro.knowledge` — `aggregate` (materialise `exercise_topics` over the touch set), `agent` (Protocol; `ClaudeKnowledgeAgent` `claude-opus-5`, streaming, two calls per topic — structure then pedagogy — with a hard `stop_reason == max_tokens` check that fails the job rather than persisting a partial spec; an intermittent `<parameter>` pseudo-syntax malformation is re-sampled up to twice, never unpacked; empty exercise list handled by an explicit prompt; `thinking` toggleable; offline `StubKnowledgeAgent`; prompt `m4-know-v5`), `extract` (one `EXTRACT_KNOWLEDGE` job per topic), `export`, `run` (`--all` / `--reset` snapshot-then-wipe / `--no-thinking` / per-topic timing). `zaspro.jobs.PermanentJobError` fails a job without spending its retry budget; transient errors still retry. Migrations `0008`–`0012`.

* **Every topic gets a complete spec — source material is not a constraint on what is extracted (ADR 0011 §2).** The agent uses exam exercises where they inform an item and its own knowledge of high-school mathematics where they do not; the ~3 requirements with no exercises and ~13 with no primary coverage still get full specs. Every item of every kind records `provenance` (`EXAM_TASK` / `MARKING_SCHEME` / `DISTRACTOR` / `AGENT_KNOWLEDGE`) — information for the reviewer, not a gate. No GAP outcomes; `knowledge_flags` carries only CONFLICT. `MisconceptionSource` / `source_kind` dropped (migration 0012).

...
As built (ADR 0009): `chunk_mappings` / `review_items` / `review_decisions` (migrations `0004`, `0005`). The Mapping Agent is a Protocol with `ClaudeMappingAgent` (`claude-opus-5`) and an offline `StubMappingAgent`, chosen by `ANTHROPIC_API_KEY` presence, so the whole path runs without the network. `AUTO_APPROVE_THRESHOLD = 0.80` on **mapping** confidence is the queue-entry lever, and is **provisional** — a parameter of `map_chunk`/`map_document`/`MAP_CHUNK`, not a baked constant, to be fixed from a calibration pass (`zaspro.mapping.run <arkusz> --review-all`, then work the queue; `zaspro.review.calibration` builds the agreement-vs-confidence curve, `GET /review/calibration`, dashboard `/calibration`). At or above the threshold a mapping is `AI_SUGGESTED` and not queued *except* that a permanent 3% audit sample (`review_items.audit_sample`, deterministic per chunk+prompt) is queued regardless, so the system can never auto-approve a block wholly unseen. Below it: `REVIEW_REQUIRED` with one `ReviewItem`. Only `AI_SUGGESTED` or human-`APPROVED` mappings propagate to `exercises.topic_id`. `zaspro.review.queue` does next-by-risk, `record_decision` (immutable, records prior status + frozen mapping confidence, REJECT needs a reason code), and `batch_approve` (shared topic+source, confidence ≥ 0.6). API: `zaspro.api` (FastAPI). Dashboard: `dashboard/` (Next.js App Router) — review page is one client component, keys `a`/`r`+digit/`e`+`j`/`k`/`b`/`s`, no route change between items.

...
* **One requirement ≠ one episode.** `official_requirement_code` is the legal definition of what is examinable, not a teaching unit. Teaching sits on a **teaching layer** above the requirements. M4 must not assume a 1:1 requirement→episode mapping anywhere.

As built (ADR 0012): the exercise-based knowledge layer of ADR 0010/0011 was torn out. `zaspro.knowledge` now writes **one spec per teaching section**.

* **Teaching sections.** `sections` + `section_requirements` (migration 0013), seeded from `seeds/teaching_sections.yaml` (proposed in `m4/teaching_tree_proposal.md`): 62 lesson-sized sections in teaching order, each covering one or more `official_requirement_code`s, together covering every podstawowy requirement at least once (`seed_sections` asserts coverage, not a partition — X.5 spans four sections by solid type, III.4 spans two by equations vs inequalities, all keeping the code). This is the teaching layer; requirements stay the legal definition.
* **The agent writes each section as a textbook chapter would** — definitions, concepts, formulas with conditions, methods with when-to-use, worked examples that build in difficulty, learning objectives, and the mistakes students make. `ClaudeSectionAgent.write` (one call, one `record_section` tool, `claude-opus-5`, streaming, `max_tokens=64000`), from the model's subject knowledge, bounded by the section scope. **Exercises are not a source** — no aggregation, no citation, no provenance, no flags; a section with no exam tasks gets exactly as full a spec as one with twenty.
* **No retry logic.** A failed section call fails the job; `zaspro.knowledge.write` prints the error and moves on; re-run by slug.
* **One review card per section.** `KNOWLEDGE_SPEC` `ReviewItem` (`ref_table = "sections"`); APPROVE marks non-rejected items `APPROVED`, REJECT throws the spec out, EDIT toggles items. `section_specs` is one row per section (last write + approval/export). Dashboard: `/knowledge` index + the `KNOWLEDGE_SPEC` branch of the review card.
* **Git holds the record.** An approved section exports to `knowledge/sections/<slug>.yaml` — one file per section; the committed file is the freeze (`write_section` refuses to re-run a frozen section without `force=True`). `scripts/backup.sh` / `restore.sh` are convenience only; `backups/` gitignored.
* **Not M5:** exercise generation, verification, normalisation. The section spec has no fields for them and nothing is built toward them.

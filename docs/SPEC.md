...
* **Teaching sections.** `sections` + `section_requirements` (migration 0013), seeded from `seeds/teaching_sections.yaml` (proposed in `m4/teaching_tree_proposal.md`): 50 lesson-sized sections in teaching order, each covering one or more `official_requirement_code`s, together covering every podstawowy requirement exactly once (`seed_sections` asserts it). This is the teaching layer; requirements stay the legal definition.
